<li class="product-attribute-list-ireloca" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>">
    <div class="dokan-product-attribute-heading">
        <span><strong><?php echo ! empty( $attribute_label ) ? esc_html( $attribute_label ) : _e( 'Attribute Name', 'dokan' ) ; ?></strong></span>
    </div>
        ...
        <!--    <label for="" class="form-label"><?php// _e( 'Value(s)', 'dokan' ); ?></label> -->
			...
			<?php if ( $attribute['is_taxonomy'] ) : ?>
				<?php if ( 'select' === $attribute_taxonomy->attribute_type ) : ?>
					...
					<select multiple="multiple" style="width:100%" data-placeholder="<?php esc_attr_e( 'Select terms', 'dokan' ); ?>" class="dokan_attribute_values dokan-select2" name="attribute_values[<?php echo $i; ?>][]" required>
						<?php
						$args = array(
							'orderby'    => 'name',
							'hide_empty' => 0
