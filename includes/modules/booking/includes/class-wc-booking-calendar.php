...
class Dokan_WC_Bookings_Calendar {
    ...
    /*print*/
  	public function dokan_print_bookings() {
  		$bookings_by_time = array();
  		$all_day_bookings = array();
  		$unqiue_ids       = array();
  		...
  		foreach ( $all_day_bookings as $booking ) {
  		  echo '<li>' ;
  			echo '<strong>#' . $booking->id . '&nbsp;' ;
  		  if ( $product = $booking->get_product() ) {
  		  echo '<br>' . $product->get_title();}
  		  echo '</strong>&nbsp;&nbsp;-&nbsp;&nbsp;'.__( 'All day', 'dokan-wc-booking' ) ;
  		  if ( ( $customer = $booking->get_customer() ) && ! empty( $customer->name ) ) {
  				...
  				echo '<br>' . __( 'Booked by', 'dokan-wc-booking' ) . ' ' . $customer->name;
  					if ( ! empty( $country ) ) {
  						echo '' . __( ' from ', 'dokan-wc-booking' ) . ' ' . $country;
  					}
  				echo '<br>' . __( 'Phone: ', 'dokan-wc-booking' ) . ' ' . $phone;
  				echo '<br>' . __( 'E-Mail: ', 'dokan-wc-booking' ) . ' ' . $customer->email;
  		    if ( $booking->has_persons() ) :

  		          $saved_persons = get_post_meta( $booking->get_id(), '_booking_persons', true );

  		          if ( ! empty ( $product ) ) {
  		              $person_types = $product->get_person_types();
  		              if ( ! empty( $person_types ) && is_array( $person_types ) ) {
  		                  echo  '<br>'.__( 'Person(s) :', 'dokan-wc-booking' ) ;

  		                  foreach ( $person_types as $person_type ) {
  		                      $person_count = ( isset( $saved_persons[ $person_type->ID ] ) ? $saved_persons[ $person_type->ID ] : 0 );
  		                      echo '<br>&nbsp;&nbsp;' . $person_type->post_title. ' : '.$person_count;
  		                  }
  		              } else if ( empty( $person_types ) && ! empty( $saved_persons ) && is_array( $saved_persons ) ) {
  		                  foreach ( $saved_persons as $person_id => $person_count ) {
  		                      echo '<br>'.__( 'Person(s) :', 'dokan-wc-booking' ).$person_count;
  		                  }
  		              }
  		          }
  		    endif;
  		    if ( $resource = $booking->get_resource() )
  		  	echo '<br>' . $resource->post_title;
  		  	echo '</li><br>';
  			}
  		  echo '</span></a></li>';
  		  $column++;
  		...
  		foreach ( $bookings_by_time as $bookings ) {
  		  foreach ( $bookings as $booking ) {
  		    ...
  		    $edit_url = add_query_arg( 'booking_id', $the_booking->id, $booking_details_url );
  		    ...
  		    if ( $last_end > $start_time )
  		      ...
  		      $column = $start_column;
  					...
  					echo '<strong>#' . $booking->id . '&nbsp;' ;
  				  if ( $product = $booking->get_product() ) {
  				  echo '<br>' . $product->get_title() . '</strong>';}
