<?php
	...
	if ( ! isset( $availability['type'] ) ) {
		$availability['type'] = 'time:range';
	}

	if ( ! isset( $availability['priority'] ) ) {
		$availability['priority'] = 9;
	}
?>
<tr>
	<td class="sort advanced_text">&nbsp;</td>
	<td style="display:none;">
		<div class="select wc_booking_availability_type">
			<select name="wc_booking_availability_type[]">
				<option style="display:none;" value="custom" <?php selected( $availability['type'], 'custom' ); ?>><?php _e( 'Date range', 'dokan' ); ?></option>
				<option style="display:none;" value="months" <?php selected( $availability['type'], 'months' ); ?>><?php _e( 'Range of months', 'dokan' ); ?></option>
				<option style="display:none;" value="weeks" <?php selected( $availability['type'], 'weeks' ); ?>><?php _e( 'Range of weeks', 'dokan' ); ?></option>
				<option style="display:none;" value="days" <?php selected( $availability['type'], 'days' ); ?>><?php _e( 'Range of days', 'dokan' ); ?></option>
				 <!-- <optgroup label="<?php  //_e( 'Time Ranges', 'dokan' ); ?>"> -->
					<option style="display:none;" value="time" <?php selected( $availability['type'], 'time' ); ?>><?php _e( 'Time Range (all week)', 'dokan' ); ?></option>
	...
	<div class="bookings-datetime-select-from">
		...
		<div class="ireloca-date-field">
			<label class="form-label"><?php _e( 'Date', 'dokan' ); ?></label>
		<div class="from_date">
			<?php
			$from_date = '';
	...
	<div class="ireloca-time-field">
		<label class="form-label"><?php _e( 'Time', 'dokan' ); ?></label>
		<div class="from_time">
	...
	<td>
		<div class="select advanced_text">
			<label class="form-label"><?php _e( 'Bookable', 'dokan' ); ?></label>
	...
	<td class="remove advanced_text">&nbsp;</td>
</tr>
